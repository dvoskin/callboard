...
function forwardRingCXReports_() {
  // Read the tunables DEFENSIVELY.
  //
  // This file has been broken three times by a partial paste -- the body
  // replaced while the settings block above it stayed on the previous version.
  // The last time cost a day: "ReferenceError: MAX_THREADS is not defined",
  // which reads like a bug in code that is in fact simply not all there.
  //
  // So the body carries its own defaults and only PREFERS the constants above.
  // A half-pasted file now runs correctly instead of failing, and pasting the
  // whole thing still lets the settings block win.
  const maxThreads = (typeof MAX_THREADS !== 'undefined') ? MAX_THREADS : 12;
  const attOpts = (typeof ATT_OPTS !== 'undefined') ? ATT_OPTS
                : { includeInlineImages: false, includeAttachments: true };

  const done = getOrCreateLabel_(LABEL_DONE);
  const fail = getOrCreateLabel_(LABEL_FAIL);
  const map = seen_();
  const threads = GmailApp.search(QUERY, 0, maxThreads);  // no label filter — see header
  ...
  threads.forEach(function (thread) {
    ...
    thread.getMessages().forEach(function (msg) {
      ...
      const atts = msg.getAttachments(attOpts).filter(function (a) {
...
function testOnce() {
  ...
  threads.forEach(function (t) {
    ...
    t.getMessages().forEach(function (m) {
      const names = m.getAttachments(attOpts).filter(function (a) {
